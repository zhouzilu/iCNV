...
```
  ...
  First row is the rsid (Name), followed by chromosome (Chr) and position (POS), then the log R ratio (sample1.Log R Ratio) and BAF (sample1.B Allele Freq). We could use function [get_array_input](https://github.com/zhouzilu/iCNV/blob/master/R/get_array_input.R) to convert into iCNV input. For example, in demo:
...
We could also plot information in a single individual using function [plotindi](https://github.com/zhouzilu/iCNV/blob/master/R/plotindi_function.R). Example shows below:
