...
For researchers work on WGS data, you need to generate your own BED file for CODEX normalization. We made a function [bed_generator.R](https://github.com/zhouzilu/iCNV/blob/master/utils/bed_generator.R) to help you with this.

After you have BAM file and BED file available, follow code below (code modified from https://github.com/yuchaojiang/CODEX with permission):
```r
###################################################
...
save(sampname_qc,ngs_plr,ngs_plr.pos,file=paste0(projectname,'_plrObj_',chr,'_',optK,'.rda'))
```
...
We could further extract variants BAF info from vcf file by applying [bambaf_from_vcf.R](https://github.com/zhouzilu/iCNV/blob/master/utils/bambaf_from_vcf.R). 
```r
library(iCNV)
dir='demo/ngs/baf' # PATH/TO/FOLDER
bambaf_from_vcf(dir,'vcf.sample.list') # not working for this example due to vcf only contain chr22
...
rs6010348,22,15462739,-0.096390619874,0.0443964861333
rs4321465,22,15520991,-0.154103130102,0.963218688965
rs4145526,22,15780940,-0.110297381878,0.0457459762692
rs12157537,22,15863717,-0.21270519495,0.957377910614
rs9604821,22,16532045,-0.0330782271922,0.0300635993481
```
First row is the rsid (Name), followed by chromosome (Chr) and position (POS), then the log R ratio (sample1.Log R Ratio) and BAF (sample1.B Allele Freq). We could use function [icnv_array_input.R](https://github.com/zhouzilu/iCNV/blob/master/utils/icnv_array_input.R) For example:
```r
###################################################
### 2.4 code chunk number 1: icnv_array_input
###################################################
library(iCNV)
dir='demo/array' # PATH/TO/FOLDER
pattern=paste0('*.csv.arrayicnv$')
icnv_array_intput(dir,pattern,chr=22)
load(file.path(dir,'icnv_array_input_22.rda'))
str(snp_lrr)
str(snp_lrr.pos)
str(snp_baf)
str(snp_baf.pos)
...
save(lrr.svd,file=paste0('array_icnv_input_svd_',chr,'.rda'))

pdf(paste0('array_icnv_input_svd_',chr,'.pdf'),height=8,width=10)
...
```r
###################################################
### 2.4 code chunk number 3: normalization
###################################################
library(iCNV)
optK = 5
D=diag(lrr.svd$d)
...
save(snp_lrr,snp_baf,snp_lrr.pos,snp_baf.pos, file=file.path(dirPath,paste0('icnv_array_input_',chr,'_svded.rda')))
```
...
library(iCNV)
# load array data
dir='demo/array' # PATH/TO/ARRAYDATA
load(file.path(dir,'icnv_array_input_22.rda'))

# load NGS PLR
load('demo/ngs/sample_plrObj_22_2.rda')

# load NGS BAF
dir='demo/ngs/baf' # PATH/TO/NGSBAF
load(file.path(dir,'bambaf_22.rda'))
