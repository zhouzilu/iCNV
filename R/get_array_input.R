...
get_array_intput=function(dir,pattern,chr=NULL,projectname=''){
	if(is.null(chr)){
		for (chr in 1:22){
			...
			snp_lrr = sapply(signal.DT,function(t){t[[4]][t$Chr==chr]},simplify = FALSE)
			names(snp_lrr)=signalFile
			snp_baf = sapply(signal.DT,function(t){t[[5]][t$Chr==chr]},simplify = FALSE)
			names(snp_baf)=signalFile
			snp_lrr.pos = lapply(seq_len(length(snp_lrr)), function(i) signal.DT[[1]]$POS[signal.DT[[1]]$Chr==chr])
			snp_baf.pos = lapply(seq_len(length(snp_baf)), function(i) signal.DT[[1]]$POS[signal.DT[[1]]$Chr==chr])
